...
/**
 * Create a contact for Colin Loader if it doesn't exist
 */
async function createColinLoaderContact() {
  try {
    log('Checking if Colin Loader contact exists...');
    ...
    if (contact) {
      log('Colin Loader contact already exists');
      return contact.contact_id;
    }
    
    // Create contact
    log('Creating Colin Loader contact...');
    const { data: newContact, error: createError } = await supabase
      .from('contacts')
      .insert([{
        name: 'Colin Loader',
        phone_number: '+971565401583',
        email: 'colin.loader@example.com',
    ...
    log(`Created Colin Loader contact with ID ${newContact.contact_id}`);
    return newContact.contact_id;
  } catch (error) {
    log(`Error creating Colin Loader contact: ${error.message}`);
    return null;
  }
}
...
async function main() {
  try {
    ...
    // Create Colin Loader contact if it doesn't exist
    const contactId = await createColinLoaderContact();
    if (!contactId) {
      log('Failed to create or find Colin Loader contact');
    }
