...
export async function POST(request: Request) {
  try {
    ...
    // Make calls to VAPI API (limit to 10 at a time to avoid rate limiting)
    const callPromises = phoneNumbers.slice(0, 10).map(async (phoneNumber) => {
      try {
        ...
        try {
          await fetch('http://localhost:3004/api/calls', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
