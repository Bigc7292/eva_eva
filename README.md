...
Quick steps to run in production-like environments.

- Use `.env.production` or the hosting provider's secret storage. Copy `.env.example` and fill values.
- Build the monorepo and start the Next.js frontend (works with the included Dockerfile):
   ...
   - Build and run with Docker (root Dockerfile):

      ```powershell
      docker build -t eva-col:latest .
      docker run -e PORT=3004 -p 3004:3004 --env-file .env.production eva-col:latest
      ```

- CI: A GitHub Actions workflow at `.github/workflows/ci.yml` builds the project and can publish a container image to GitHub Container Registry when changes are pushed to `main`/`master`.

- Webhooks: ensure `NEXT_PUBLIC_APP_URL` is set to your production domain so VAPI webhooks are registered to the correct URL (`/api/webhooks/vapi`).

If you'd like, I can:

- Add a small health-check endpoint and systemd/unit file for a Linux server.
- Configure environment-specific Next.js settings (rewrites, headers) for production.
